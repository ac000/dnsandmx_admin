...
unsigned long long log_login(void)
{
	...
	sql_query(conn, "INSERT INTO utmp VALUES (%ld.%06ld, %u, '%s', '%s', "
			"'%s', %llu)",
...
void create_session(unsigned long long sid)
{
	...
	username = make_mysql_safe_string(get_var(qvars, "username"));
	res = sql_query(conn, "SELECT uid, gid, name, capabilities FROM "
			"passwd WHERE username = '%s'", username);
	...
	fcgx_p("Set-Cookie: session_id=%s; path=/; httponly; secure;\r\n",
			session_id);
